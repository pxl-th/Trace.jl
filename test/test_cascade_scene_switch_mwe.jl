...
const _BE_M7  = Lava.LavaBackend()
const _CTX_M7 = Lava.vk_context()
...
if get(ENV, "LAVA_RUN_CASCADE_REPRO", "0") == "1"
    @testset "MWE-7: A→B scene switch (cascade fix regression)" begin
        sa = _build_a(_BE_M7)
        _render_one(sa)
        @test !Lava.device_lost(_CTX_M7)  # A render must be clean
        ...
        crashed = crashed || Lava.device_lost(_CTX_M7)
