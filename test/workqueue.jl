...
@testset "WorkQueue" begin
    ...
    # Draining a queue used to be `foreach(f, queue, args...)`, which launched
    # `workqueue_map_kernel!` over a host-read or Lava-indirect count. It is a
    # graph pass now — `dispatch!` with `Mantle.DeviceRange(q.size)` — so the
    # coverage lives with the rest of the graph, in `test_mantle_device.jl`.
end
