...
@inline function unoccluded(t::VisibilityTester, scene::Scene)::Bool
    !intersect_p(t.pool, scene, spawn_ray(t.pool, t.p0, t.p1))
end

function trace(t::VisibilityTester, scene::Scene)::RGBSpectrum
    ray = spawn_ray(t.pool, t.p0, t.p1)
    s = RGBSpectrum(1f0)
    while true
        hit, primitive, interaction = intersect!(t.pool, scene, ray)
        ...
        !hit && break
        ray = spawn_ray(t.pool, interaction, t.p1)
    end
    s
end
